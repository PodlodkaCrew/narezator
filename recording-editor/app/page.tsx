...
export default function Home(){
 ...
 useEffect(()=>{
  fetch('/recording.json').then(r=>{if(!r.ok)throw Error('Cannot load the recording transcript.');return r.json() as Promise<Recording>}).then((d:Recording)=>{d.words.sort((a,b)=>a.start-b.start||a.id-b.id);setData(d)}).catch(e=>setLoadError(e.message));
  fetch('/api/media-info').then(r=>r.json() as Promise<{burnedIn:boolean}>).then(d=>setBurned(d.burnedIn)).catch(()=>{});
  fetch('/api/waveform').then(r=>r.json() as Promise<{peaks:number[]}>).then(d=>setPeaks(d.peaks||[])).catch(()=>{});
  fetch('/api/export').then(r=>r.json() as Promise<{jobs:ExportJob[]}>).then(d=>{if(d.jobs?.length)setJob(d.jobs.at(-1)||null)}).catch(()=>{});
 },[]);
 const jobId=job?.id,jobStatus=job?.status;
 useEffect(()=>{
  if(!jobId||!['queued','rendering'].includes(jobStatus||''))return;
  const interval=setInterval(()=>fetch(`/api/export/${jobId}`).then(r=>r.json() as Promise<ExportJob>).then(setJob).catch(()=>setExportError('Cannot read export progress. Reconnect to the local editor.')),1500);
 ...
 async function startExport(){
  ...
  try{const response=await fetch('/api/export',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({project:exportProject,burn:burnExport,height:Number(exportHeight),title:activeReel?.title||data?.title,kind:activeReel?'reel':'main'})});const result=await response.json() as {id:string;error?:string};if(!response.ok)throw Error(result.error||'Export could not start.');setJob({id:result.id,status:'queued',progress:0,message:`Preparing ${activeReel?'reel':'export'}`})}catch(e){setExportError((e as Error).message)}
 }
 if(loadError||(!project&&error))return <div className="startup"><Scissors/><h1>Could not open the editor</h1><p>{loadError||error}</p><Button onClick={()=>location.reload()}>Try again</Button></div>;
 ...
 return <main className="editor">
  ...
  <div className="workspace">
   ...
   <section className="viewer-panel"><div className="panel-heading">{activeReel?<Clapperboard size={15}/>:<Film size={15}/>}<strong>{activeReel?activeReel.title:'Viewer'}</strong><div className="mode-switch"><button className={mode==='edit'?'active':''} onClick={()=>switchMode('edit')}>{activeReel?'This reel':'Your cut'}</button><button className={mode==='source'?'active':''} onClick={()=>switchMode('source')}>Source</button></div></div>
    <div ref={frame} className="video-frame"><video ref={video} src="/api/media" preload="auto" playsInline muted={muted} onLoadedMetadata={()=>{mediaReady.current=true;seek(viewTime.current)}} onPlay={()=>setPlaying(true)} onPause={()=>setPlaying(false)} onEnded={()=>{if(modeRef.current==='edit')advance()}} onError={()=>notify('The video could not load. Check that the local editor is running.')} onClick={togglePlay}><track kind="captions" src="/source-captions.vtt" srcLang="ru" label="Русский"/></video>
