...
function download(name:string,text:string,type='application/json'){
 void type;void window.narezator.saveText({suggestedName:name,text});
}
...
export default function Home(){
 ...
 const {project,update,status,error,flush,hasRecovery,recoveryText}=useProject(data);
 const [mode,setMode]=useState<ViewMode>('edit'),[clock,setClock]=useState(0),[sourceClock,setSourceClock]=useState(0),[playing,setPlaying]=useState(false);
 ...
 useEffect(()=>{
  window.narezator.workspaceInfo().then(async info=>{
   setWorkspaceInfo(info);if(!info.active)return;
   const recording=await window.narezator.loadRecording();recording.words.sort((a,b)=>a.start-b.start||a.id-b.id);setData(recording);
   window.narezator.mediaInfo().then(d=>setBurned(d.burnedIn)).catch(()=>{});
   window.narezator.loadWaveform().then(d=>setPeaks(d.peaks||[])).catch(()=>{});
   window.narezator.listExports().then(d=>{if(d.jobs?.length)setJob(d.jobs.at(-1)||null)}).catch(()=>{});
  }).catch(e=>setLoadError(e.message));
 },[]);
 const jobId=job?.id,jobStatus=job?.status;
 useEffect(()=>{
  if(!jobId||!['queued','rendering'].includes(jobStatus||''))return;
  const interval=setInterval(()=>window.narezator.getExport(jobId).then(setJob).catch(()=>setExportError('Cannot read export progress.')),1500);
 ...
 async function saveAnnotations(){
  ...
  try{const result=await window.narezator.saveText({suggestedName:'annotations.md',text:buildAnnotationReport(d.title,p.source,p.annotations,d.words)});if(!result.cancelled)notify('Annotations exported as Markdown.')}catch(error){notify((error as Error).message)}
 }
 function saveReport(){
 ...
 async function startExport(){
  ...
  try{
   const result=exportScope==='all-reels'
    ?await window.narezator.startReelsExport({project,burn:burnExport,height:Number(exportHeight)})
    :await window.narezator.startExport({project:{...project,clips},burn:burnExport,height:Number(exportHeight),title:activeReel?.title||data?.title||'Narezator export',kind:activeReel?'reel':'main'});
 ...
 if(workspaceInfo?.active===false||loadError||(!project&&error))return <div className="startup"><Scissors/><h1>Narezator projects</h1><p>{loadError||error||'Open a saved project or start a new recording.'}</p><ProjectControls info={workspaceInfo} welcome prepare={prepareSwitch}/></div>;
 ...
 return <main className="editor">
  <header className="app-header"><div className="brand"><span className="brand-icon"><Scissors size={19}/></span><span className="brand-wordmark">Narezator</span><span className="brand-divider"/><span className="project-name" title={data.title}>{data.title}</span></div>
  ...
  {error&&<div className="error-banner">{error} {hasRecovery&&<button onClick={()=>download('narezator-recovery.json',recoveryText())}>Download recovery</button>}</div>}
  <div className="workspace">
   ...
   <section className="viewer-panel"><div className="panel-heading">{activeReel?<Clapperboard size={15}/>:<Film size={15}/>}<strong>{activeReel?activeReel.title:'Viewer'}</strong><div className="mode-switch"><button className={mode==='edit'?'active':''} onClick={()=>switchMode('edit')}>{activeReel?'This reel':'Your cut'}</button><button className={mode==='source'?'active':''} onClick={()=>switchMode('source')}>Source</button></div></div>
    <div ref={frame} className="video-frame"><video ref={video} src={window.narezator.mediaUrl} preload="auto" playsInline muted={muted} onLoadedMetadata={()=>{mediaReady.current=true;seek(viewTime.current)}} onPlay={()=>setPlaying(true)} onPause={()=>setPlaying(false)} onEnded={()=>{if(modeRef.current==='edit')advance()}} onError={()=>notify('The video could not load. Choose the original video again.')} onClick={togglePlay}><track kind="captions" src={window.narezator.captionsUrl} srcLang="ru" label="Русский"/></video>
  ...
  <Dialog open={exportOpen} onOpenChange={setExportOpen}><DialogContent className="export-dialog">
   ...
   {job&&<div className="export-progress"><div><strong>{job.message}</strong><span>{Math.round(job.progress*100)}%</span></div><progress value={job.progress} max="1"/>
    {['queued','rendering'].includes(job.status)&&<Button variant="ghost" size="sm" onClick={()=>void window.narezator.cancelExport(job.id).then(()=>window.narezator.getExport(job.id)).then(setJob).catch(e=>setExportError(e.message))}>Cancel export</Button>}
    {job.kind==='reels'&&job.outputFolder?<><p className="export-note">{job.completed||0} of {job.total} reels saved{job.status==='cancelled'||job.status==='error'?'. Completed reels remain in the export folder.':'.'}<br/>{job.outputFolder}</p><div className="export-downloads"><button onClick={()=>void window.narezator.revealExport(job.id).catch(e=>setExportError(e.message))}>Show folder</button></div></>:job.status==='done'&&<div className="export-downloads"><button onClick={()=>void window.narezator.saveExportFile(job.id,'edited-video.mp4')}><Download size={14}/>{job.kind==='reel'?'Reel':'Video'}</button><button onClick={()=>void window.narezator.saveExportFile(job.id,'cut-report.md')}>Cut report</button><button onClick={()=>void window.narezator.saveExportFile(job.id,'transcript.txt')}>Transcript</button><button onClick={()=>void window.narezator.saveExportFile(job.id,'subtitles.srt')}>Captions</button><button onClick={()=>void window.narezator.saveExportFile(job.id,'edit-list.json')}>Edit list</button><button onClick={()=>void window.narezator.revealExport(job.id)}>Show folder</button></div>}
   </div>}
