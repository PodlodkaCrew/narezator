...
export default function ProjectControls({info,welcome=false,prepare}:{info:WorkspaceInfo|null;welcome?:boolean;prepare:()=>Promise<boolean>}){
 ...
 const buttons=<><Button disabled={busy} onClick={()=>{setCreating(true);setError('')}}>New Project</Button><Button variant="outline" disabled={busy} onClick={()=>void action(()=>window.narezator.openWorkspace())}>Open Project…</Button><Button variant="outline" disabled={busy} onClick={()=>void action(()=>window.narezator.importWebProject())}>Import Web Project…</Button>{info?.active&&<Button variant="ghost" disabled={busy} onClick={()=>void action(()=>window.narezator.closeWorkspace())}>Close Project</Button>}</>;
 return <>
  ...
  <Dialog open={creating} onOpenChange={open=>{if(!busy)setCreating(open)}}><DialogContent className="export-dialog new-project-dialog"><DialogHeader><DialogTitle>New project</DialogTitle><DialogDescription>Choose a video, add its transcript and chapters, then choose where to save the project.</DialogDescription></DialogHeader>
   ...
   <div className="dialog-actions"><Button variant="outline" disabled={busy} onClick={()=>setCreating(false)}>Cancel</Button><Button disabled={busy||!files.videoPath||!title.trim()} onClick={()=>void action(()=>window.narezator.newWorkspace({...files,title}))}>{busy?'Creating…':'Create project…'}</Button></div>
