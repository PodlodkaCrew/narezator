...
async function openWorkspace(file){
 if(!file){const result=await dialog.showOpenDialog(window,{title:'Open Narezator project',properties:['openFile'],filters:[{name:'Narezator project',extensions:['narezator','cutroom']}]});if(result.canceled)return {cancelled:true};file=result.filePaths[0]}
...
function registerIpc(){
 ...
 ipcMain.handle('media:info',()=>({burnedIn:false,source:workspace().recording.source,duration:workspace().recording.duration,url:'narezator-media://video/source'}));
...
function createWindow(){
 window=new BrowserWindow({width:1500,height:980,minWidth:980,minHeight:700,backgroundColor:'#2c3b44',title:'Narezator',webPreferences:{preload:path.join(__dirname,'preload.cjs'),contextIsolation:true,nodeIntegration:false,sandbox:true}});
 const dev=process.env.NAREZATOR_DEV_URL||process.env.CUTROOM_DEV_URL;if(dev)window.loadURL(dev);else window.loadFile(path.join(__dirname,'..','dist','renderer','index.html'));
}
