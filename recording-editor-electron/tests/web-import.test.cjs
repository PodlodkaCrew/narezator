...
function fixture(){
 const root=fs.mkdtempSync(path.join(os.tmpdir(),'cutroom-web-import-'));
 ...
 const original=store.create({manifestPath:path.join(root,'original.cutroom'),videoPath,recording});
...
test('web import discovers assets, copies latest edits/reels/history and keeps both originals untouched',async()=>{
 const f=fixture();try{
  ...
  const dialog={showOpenDialog:async()=>({canceled:false,filePaths:[f.editsPath]}),showSaveDialog:async()=>{
   fs.writeFileSync(f.editsPath,JSON.stringify(latest));return {canceled:false,filePath:path.join(f.root,'imported.cutroom')};
  }};
