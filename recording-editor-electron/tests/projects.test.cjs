...
void test('switching and closing retain independent edits, reels, history and project identity',()=>{
 const dir=fs.mkdtempSync(path.join(os.tmpdir(),'cutroom-projects-'));
 try{
  ...
  const a=store.create({manifestPath:path.join(dir,'a.cutroom'),videoPath,recording,project});
  ...
  const b=store.create({manifestPath:path.join(dir,'b.cutroom'),videoPath,recording:{...recording,title:'B'}});
  ...
  fs.writeFileSync(path.join(dir,'invalid.cutroom'),'{}');assert.throws(()=>restarted.open(path.join(dir,'invalid.cutroom')));assert.equal(restarted.identity(),b.id);
 }finally{fs.rmSync(dir,{recursive:true,force:true})}
});
void test('existing Electron project remains in place and appears in Recent Projects',()=>{
 const dir=fs.mkdtempSync(path.join(os.tmpdir(),'cutroom-legacy-'));
 try{
...
void test('new recording uses selected video and subtitles instead of bundled metadata',async()=>{
 const dir=fs.mkdtempSync(path.join(os.tmpdir(),'cutroom-import-'));
 try{
