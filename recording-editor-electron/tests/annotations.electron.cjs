...
const root=fs.mkdtempSync(path.join(os.tmpdir(),'cutroom-annotations-'));app.setPath('userData',path.join(root,'profile'));
...
const store=new ProjectStore({userData:path.join(root,'profile'),resourcesPath:root,legacyRoot:root});store.create({manifestPath:path.join(root,'notes.cutroom'),videoPath,recording:data,project});
...
app.whenReady().then(async()=>{
 ...
 try{
  protocol.handle('cutroom-media',r=>new URL(r.url).hostname==='video'?mediaResponse(r,videoPath):new Response('WEBVTT\n',{headers:{'Content-Type':'text/vtt'}}));
