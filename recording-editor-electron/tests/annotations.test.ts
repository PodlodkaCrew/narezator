...
test('annotations persist through saves, close/reopen and copied projects without altering cuts',()=>{
 const dir=mkdtempSync(join(tmpdir(),'cutroom-notes-'));try{
  ...
  const p={...createProject(data),annotations:[note]};const config=store.create({manifestPath:join(dir,'a.cutroom'),videoPath,recording:data,project:p});
  ...
  const bytes=readFileSync(config.projectPath);store.create({manifestPath:join(dir,'b.cutroom'),videoPath,recording:data,project:store.load().project});assert.deepEqual(store.load().project.annotations,[{...note,text:'Updated'}]);assert.deepEqual(readFileSync(config.projectPath),bytes);
