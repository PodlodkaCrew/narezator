...
async function importWebProject({store,dialog,window}){
 ...
 const defaultPath=firstFile([old?.importedFrom, path.join(store.legacyRoot,'edits','recording.edits.json')].filter(Boolean));
 ...
 if(!recordingPath){
  const result=await dialog.showOpenDialog(window,{title:'Locate web transcript — recording-editor/public/recording.json',defaultPath:dir,properties:['openFile'],filters:[{name:'Recording metadata',extensions:['json']}]});
 ...
 const result=await dialog.showSaveDialog(window,{title:'Save imported Cutroom project',defaultPath:recording.title+'.cutroom',filters:[{name:'Cutroom project',extensions:['cutroom']}]});
