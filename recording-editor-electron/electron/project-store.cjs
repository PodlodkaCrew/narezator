...
class ProjectStore{
 ...
 ensureManifest(){
  ...
  const recording=this.recording(config),manifestPath=path.join(this.userData,'project','current.cutroom');
  const next={...config,id:'legacy',title:recording.title,manifestPath,projectPath:this.legacyProjectPath};
  atomicJson(manifestPath,{format:'cutroom',version:1,...next});this.writeConfig(next);this.remember(next);return next;
 }
 ...
 create({manifestPath,videoPath,recording,project,waveformPath,captionsPath}){
  ...
  atomicJson(manifestPath,{format:'cutroom',version:1,...relative});
  this.writeConfig(config);this.remember(config);return config;
 }
 inspect(manifestPath){
  manifestPath=path.resolve(manifestPath);
  const raw=JSON.parse(fs.readFileSync(manifestPath,'utf8'));
  if(raw.format!=='cutroom'||raw.version!==1||typeof raw.id!=='string')throw Error('Choose a .cutroom project file. To transfer browser edits, use Import Web Project.');
 ...
 open(manifestPath,replacementVideo){
  ...
  this.ensureManifest();
  if(replacementVideo)atomicJson(manifestPath,{format:'cutroom',version:1,...config});
  this.writeConfig(config);this.remember(config);return config;
 }
