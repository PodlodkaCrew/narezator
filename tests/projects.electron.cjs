...
const store=new ProjectStore({userData:path.join(root,'profile'),resourcesPath:root,legacyRoot:root});const original=store.create({manifestPath:path.join(root,'original.narezator'),videoPath:video,recording,project});
...
app.whenReady().then(async()=>{
 ...
 try{
  ...
  await delay(20);await click('Close Project');await until("document.body.innerText.includes('Narezator projects')");
